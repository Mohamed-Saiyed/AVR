 ...
 * CFile1.c
 *
 * Created: 5/10/2020 4:53:16 AM
 *  Author: Mohamed-Sayed
 */ 
//AT COMMANDS
#include "esp_Cleint.h"
...
volatile  uint16 Counter = 0; //data counter
char  rx_buffer[500]; //storing response
volatile char temp;//temporary storing variable
char Data_Send[4];
uint16 Data_Send_Value;
char Value_txt[6];

static char find_string(char *string , char *string_nedded);
static char Response_success();
static void Get_String(char *source,char *StartFrom,char Delete ,char *EndString ,char add,char *SaveTOanArray);
static void Get_IP_MAC();
static void reset_buffer();
static void Wifi_Mode();
static void Wifi_Network();
static void WiFi_Reset();
static void Wifi_Connection_Mode();
static void wifi_disconnect();
static void Wifi_Send_Date(uint16 send_data_size , char *Send_Command);
static void Wifi_Close();
static void Tcb_Cleint_Config();
static void ltrim(char *string);
static void delay_ms(uint8_t milliseconds);
 void esp_callback()
{
	sei();
...
void UART_Set_Callback(esp_callback);
static char find_string(char *string , char *string_nedded)
{
	int match = 0;
